...
public class Action
{
    ...
    public Action(TextureManager textureManager, string name, Slot slot, byte level, string type, int frame, string source, double cooldown, byte sound, int fromani, int fromanispeed, int toani, int toanispeed, bool checkIfHidden, bool castMsg, bool checkSpellBar, bool allowRefresh, int seconds, string startMsg, string endMsg, byte bodyAniType, ushort bodyAniSpeed, int baseDmg, string element, string targettype, string status)
    {
        _status = status;
        _fromani = fromani;
        ...
        _frame = frame;
        frame--;
        _sprite.Texture = _textureManager.Get(type + "_F" + frame + text + "_C0", ".epf", source);
